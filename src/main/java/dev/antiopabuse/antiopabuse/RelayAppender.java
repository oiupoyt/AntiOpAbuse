...
public final class RelayAppender extends AbstractAppender {
    ...
    public RelayAppender(WebhookDispatcher dispatcher, Logger javaLogger) {
        // Use a simple pattern layout – but we will NOT use toSerializable().
        // We read directly from the LogEvent to avoid Paper's custom message wrappers.
    ...
    public void append(LogEvent event) {
        try {
            // Build the line ourselves from the event fields.
            // This is the most reliable method across all Paper/Spigot versions.
            String level   = event.getLevel().name();           // INFO, WARN, etc.
            String message = event.getMessage().getFormattedMessage(); // actual text
            ...
            // Format to match the standard console look
            String line = "[" + level + "]: " + message;

            if (MessageFilter.isAllowed(line)) {
                dispatcher.dispatch(line);
            }
        } catch (Exception e) {
            // Never let logging errors propagate into the server thread
